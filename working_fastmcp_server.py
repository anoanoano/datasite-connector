#!/usr/bin/env python3
"""
Working FastMCP HTTP Server - copying successful patterns
"""
import asyncio
import json
import logging
...
async def initialize_components():
    ...
    try:
        logger.info("Initializing DataSite components for working FastMCP server...")
...
async def list_datasets(tags: list[str] = None) -> str:
    ...
    """
    try:
        logger.info(f"Working FastMCP list_datasets called with tags: {tags}")
...
async def get_content(dataset_name: str) -> str:
    ...
    """
    try:
        logger.info(f"Working FastMCP get_content called for dataset: {dataset_name}")
...
async def search_content(query: str) -> str:
    ...
    """
    try:
        logger.info(f"Working FastMCP search_content called with query: {query}")
...
def main():
    """Main server function using FastMCP HTTP transport."""
    async def init_and_run():
        # Initialize components first
        await initialize_components()
        
        logger.info("Starting working FastMCP HTTP server on port 8082...")
        
        # Use FastMCP's HTTP transport as shown in the documentation
